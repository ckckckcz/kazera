...
export function FilePreview({ file, url, onRemove, showRemoveButton = true }: FilePreviewProps) {
  ...
  const renderPreviewContent = () => {
    ...
    if (["jpg", "jpeg", "png", "gif"].includes(extension || "")) {
      return <img src={url || "/placeholder.svg"} alt={file.name} className="max-h-[80vh] max-w-full object-contain" />
    } else if (extension === "pdf") {
      return <iframe src={`${url}#toolbar=0`} className="w-full h-[80vh]" title={file.name} />
    } else {
      return (
        <div className="flex flex-col items-center justify-center p-8">
          ...
          <p className="mt-4 text-center">
            ...
            <a href={url} download={file.name} className="block mt-2 text-primary hover:underline">
              Download file
            </a>
          </p>
        </div>
      ...
      <Dialog open={previewOpen} onOpenChange={setPreviewOpen}>
        <DialogContent className="max-w-4xl">
          <div className="flex justify-between items-center mb-4">
            ...
            <a href={url} download={file.name} className="text-sm text-primary hover:underline">
              Download
            </a>
          </div>
